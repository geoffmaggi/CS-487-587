...
/**
 * <h3>Minibase Disk Manager</h3> The disk manager layer manages a database of
 * disk pages. It provides several services based on those pages.
 * <ol>
 * <li>A logical file layer: Read and write each page and store related
 * statistics
 * <li>Allocate and deallocate disk pages. Store a space map indicating which
 * pages are currently allocated.
 * <li>Groups of pages may be organized as a file, by a higher layer.
 * </ol>
 * The disk manager manages a library of entries consisting of the name of a
 * file and the page number of the first page in the file.
 */
public class DiskMgr implements GlobalConst {
	...
	/**
	 * Pages in the database are stored as an Operating System file. This is the
	 * name of that file.
	 */
	...
	/**
	 * Close the database file. Ensure that buffer contents have been written to
	 * disk and close the OS file.
	 */
	public void closeDB() {
		try {
	 ...
	 * 
	 * @param pageno
	 *          identifies the page to read. It is the page number in the OS file.
	 *          Also referred to as the Id of the page.
	 * @param mempage
	...
	/**
	 * Allocates a run of disk pages. run = contiguous sequence of pages
	 * 
	 * @return The id of the first page in the run
	 * @throws IllegalArgumentException
	 *           if run_size is invalid
	 * @throws IllegalStateException
	 *           if there is no room for a run of that length
	 */
	...
	/**
	 * Deallocates a run of pages on disk. The bits in the space map are just set to
	 * 0. The space map is not checked to see that their values were 1, i.e. that
	 * the pages were previously allocated.
	 * 
	 * @param firstid
	 *          id of the first page to deallocate
	 * @param run_size
	 *          number of pages to deallocate
	 * @throws IllegalArgumentException
	 *           if firstid or run_size is invalid
	 */
	...
	public int getAllocCount() {
		...
		for (int i = 0; i < num_map_pages; i++) {
			...
			for (int pgptr = 0; num_bits_this_page > 0; pgptr++) { // start forloop02
				for (int mask = 1; mask < 256
						&& num_bits_this_page > 0; mask = (mask << 1), --num_bits_this_page, ++bit_number) {
	...
	/**
	 * Adds an entry to the file library. Each entry contains the name of the file
	 * and the PageId of the file's first page. The file library is stored on the
	 * first page of the database. More pages are allocated as the library grows.
	 * 
	 * @throws IllegalArgumentException
	 *           if fname or start_pageno is invalid
	 */
	...
	public void delete_file_entry(String fname) {
		...
		if (get_file_entry(fname) == null) {
			throw new IllegalArgumentException("File entry not found in library; delete entry aborted");
		}
		...
		do {
			...
			while (entry < hpage.getNumOfEntries()) {
				...
				if ((tmppid.pid != INVALID_PAGEID) && (tmpname.compareToIgnoreCase(fname) == 0)) {
					break;
				}
				entry++;
			}
	 ...
	 * 
	 * @return PageId of the file's first page, or null if the file is not in the
	 *         library
	 */
