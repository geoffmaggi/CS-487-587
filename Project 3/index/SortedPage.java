...
class SortedPage extends Page {
	...
	public short getFreeSpace() {
		return (short) (getShortValue(USED_PTR) - (HEADER_SIZE + getEntryCount() * SLOT_SIZE));
	}
	...
	public boolean insertEntry(DataEntry entry) {
		...
		if (i < slotCnt) {
			...
			System.arraycopy(data, slotpos, data, slotpos + SLOT_SIZE, (slotCnt - i) * SLOT_SIZE);
	...
	/**
	 * Deletes a data entry from the page, compacting the free space (including the
	 * slot directory).
	 * 
	...
	public boolean deleteEntry(DataEntry entry) {
		...
		System.arraycopy(data, slotpos + SLOT_SIZE, data, slotpos, (slotCnt - i - 1) * SLOT_SIZE);
