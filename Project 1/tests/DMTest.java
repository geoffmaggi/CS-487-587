	...
	protected boolean test4() {
		...
		if (status == PASS) {
			...
			try {
				...
				e.printStackTrace();
				System.err.print(
						"*** Too little space available: could not " + "allocate " + (DB_SIZE - pid) + " pages\n");
			}

			if (status == PASS) {
				if (pgid.pid != pid) {
					status = FAIL;
					System.err.print("*** Expected the first page allocated to " + "be page " + pid + "\n");
		...
		if (status == PASS) {
			System.out.print("  - At this point, all pages should be claimed.  " + "Try to allocate one more.\n");
			try {
				pgid = Minibase.DiskManager.allocate_page();
			} catch (IllegalStateException e) {
				status = FAIL;
