...
class HFTest extends TestDriver {
	...
	protected boolean test1() {
		...
		if (Minibase.BufferManager.getNumUnpinned() != Minibase.BufferManager.getNumFrames() - 1) {
		...
		scan.close();
		if (Minibase.BufferManager.getNumUnpinned() != Minibase.BufferManager.getNumFrames()) {
			System.err.println("*** The heap-file scan has not unpinned " + "its pages after finishing\n");
			status = FAIL;
		} else if (i != (FILE_SIZE)) {
			status = FAIL;
	...
	protected boolean test4() {
		...
		try {
			record = scan.getNext(rid); // This leaves a page pinned messing up other tests...
			Minibase.BufferManager.unpinPage(rid.pageno, UNPIN_CLEAN); // My code to fix it
		} catch (Exception e) {
	...
	protected boolean test5() {
		...
		for (int i = 0; (i < FILE_SIZE) && (status == PASS); i++) {
			...
			if ((rec2.ival != i) || (rec2.fval != i * 2.5)) {
				...
				System.err.println("rec.fval: " + rec2.fval + " should be " + (i * 2.5) + "\n");
